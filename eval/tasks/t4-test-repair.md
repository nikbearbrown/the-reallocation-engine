...
Tests: diagnose a failing conformance check and fix the minimal cause. (The repo's
...
block**, which the markdown conformance check flags. `eval/fixtures/` is outside the
default conformance scan, so this does not break `npm run verify` — you must point
the checker at it explicitly.
