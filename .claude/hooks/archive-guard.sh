...
# Enforces Snickerdoodle's no-delete rule (instructions/_shared/no-delete.md):
...
python3 - <<'PY'
import json
print(json.dumps({"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Snickerdoodle no-delete rule: never delete source, data, recipes, or logs — archive them instead (or your full-copy archive). Safe rm targets are rebuildables only: .build, __pycache__, *.pyc, *.bak."}}))
PY
