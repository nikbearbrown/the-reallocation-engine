...
All O*NET and BLS data lives in Neon PostgreSQL. The Next.js page queries the database; it does not parse flat files at runtime. The O*NET db-30-2 text files are a one-time import source, not a runtime dependency.
...
**How it works:** A Node.js import script reads each O*NET text file from `db-30-2-text/`, parses the tab-delimited rows, and bulk-inserts into Neon in batches of 500. Runs once; re-run when a new O*NET release ships.

**Inputs:** `db-30-2-text/*.txt` files — 40+ files covering occupations, abilities, skills, knowledge, tasks, interests, work styles, work values, related occupations, alternate titles
...
Context: db-30-2-text folder path, DATABASE_URL.
