...
- **Recipe lifecycle** — the evidence-gated status ladder in every recipe's frontmatter (`DRAFT` → `SPECIFIED` → `RUNNABLE-SAMPLE` → `RUNNABLE-LIVE` → `VERIFIED`); each transition requires a logged run, gate decision, or attestation, and editing the status without the evidence is a violation, not a promotion.
- **Human card (`.card.md`)** — the maintainer-facing half of a recipe pair: purpose, what it can and cannot verify, dependencies, and named failure modes, kept beside the agent-facing recipe so one artifact never has to serve both customers.
- **Attestation** — the human's recorded judgment of a running system: what was run, what was seen, what was expected — with a mandatory "did not test" section, because an empty one is the new "it works."
