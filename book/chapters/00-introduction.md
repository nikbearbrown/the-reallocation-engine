...
2. **Check the environment.** Run `npm run doctor`. It reports what tools are installed, which commands have real scripts behind them, and where every recipe stands in its lifecycle — an honest map of the runnable surface. Follow it with `npm run verify`, which checks that the repository itself conforms: files parse, the manifest matches the tree.
...
4. **Take the first win.** Work through `docs/tutorials/01-first-scan.md` — about thirty minutes. It walks the full loop the rest of the book assumes: configure `portals.yml`, predict what you expect, run `npm run ats:scan -- --dry-run`, read the report line by line against the live board, and log the run.
5. **Read Chapter 1.** Open `book/chapters/01-the-fluency-trap.md`. The idea you just watched the scorer enforce — that a confident output is not a correct one — is the idea the whole engine exists to defend.
